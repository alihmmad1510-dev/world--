...
app.use(cors());
app.use(express.static(__dirname)); // يخدم index.html + online.js
...
// rooms: Map<code, { hostId, createdAt, players: Map<socketId, playerData> }>
const rooms = new Map();
...
io.on('connection', (socket) => {
  ...
  socket.on('joinRoom', ({ code, playerInfo } = {}, cb) => {
    try {
      ...
      // أرسل اللاعبين الحاليين للقادم الجديد
      const existing = [];
...
/* ============================================================
   تنظيف الغرف الميتة كل 30 ثانية
   ============================================================ */
setInterval(() => {
  const now = Date.now();
...
server.listen(PORT, () => {
  console.log(`\n🚗 GTA 6 City server running on port ${PORT}`);
